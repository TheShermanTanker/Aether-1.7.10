package com.gildedgames.aether.entities.hostile;

import com.gildedgames.aether.AetherConfig;
import com.gildedgames.aether.blocks.BlocksAether;
import com.gildedgames.aether.entities.passive.EntityAetherAnimal;
import com.gildedgames.aether.entities.projectile.EntityPoisonNeedle;
...
public class EntityAechorPlant extends EntityAetherAnimal {
	...
	public void onLivingUpdate() {
		...
		if (this.worldObj.getBlock(MathHelper.floor_double(this.posX), MathHelper.floor_double(this.posY) - 1, MathHelper.floor_double(this.posZ)) != BlocksAether.aetherGrass) {
			this.setHealth(0.0F);
		}
	}
	...
	public boolean interact(EntityPlayer player) {
		...
		if (heldItem != null && !this.worldObj.isRemote) {
			if (heldItem.getItem() == ItemsAether.skyrootBucket && EnumSkyrootBucketType.getType(heldItem.getItemDamage()) == EnumSkyrootBucketType.Empty && this.poisonRemaining > 0) {
				if (--heldItem.stackSize == 0) {
					player.setCurrentItemOrArmor(0, new ItemStack(ItemsAether.skyrootBucket, 1, EnumSkyrootBucketType.Poison.meta));
				} else if (!player.inventory.addItemStackToInventory(new ItemStack(ItemsAether.skyrootBucket, 1, EnumSkyrootBucketType.Poison.meta))) {
					player.entityDropItem(new ItemStack(ItemsAether.skyrootBucket, 1, EnumSkyrootBucketType.Poison.meta), 1.0F);
				}
	...
	protected void dropFewItems(boolean var1, int var2) {
		this.dropItem(ItemsAether.aechorPetal, 2);
	}
