...
import com.gildedgames.aether.entities.effects.EffectInebriation;
...
public class ItemSkyrootBucket extends Item {
		...
		if (world.getBlock(movingobjectposition.blockX, movingobjectposition.blockY, movingobjectposition.blockZ) == Blocks.cauldron && !world.isRemote)
		{
			...
			if (EnumSkyrootBucketType.getType(meta) == EnumSkyrootBucketType.Water)
			{
				if (waterLevel < 3)
				{
					...
					{
						player.inventory.setInventorySlotContents(player.inventory.currentItem, new ItemStack(ItemsAether.skyrootBucket, 1, EnumSkyrootBucketType.Empty.meta));
					}
			...
			if (movingobjectposition.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK)
			{
				...
				if (flag)
				{
					...
					if (material == Material.water && l == 0)
					{
						...
						return this.fillBucket(heldItem, player, ItemsAether.skyrootBucket);
					}
				}
				else
				{
	...
	public ItemStack onBucketUsed(ItemStack itemstack, World world, EntityPlayer entityplayer) {
		...
		if (EnumSkyrootBucketType.getType(meta) == EnumSkyrootBucketType.Poison) {
			...
			if (!world.isRemote)
			{
				entityplayer.curePotionEffects(new ItemStack(ItemsAether.skyrootBucket, EnumSkyrootBucketType.Remedy.meta));
			}
