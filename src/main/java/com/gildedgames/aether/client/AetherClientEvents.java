...
import com.gildedgames.aether.network.packets.PacketCapeChanged;
import com.gildedgames.aether.network.packets.PacketExtendedAttack;
...
public class AetherClientEvents {
	...
	{
		return stackID == ItemsAether.valkyrieShovel || stackID == ItemsAether.valkyrieAxe || stackID == ItemsAether.valkyriePickaxe || stackID == ItemsAether.valkyrieLance;
	}
	...
	private boolean onPickEntity(MovingObjectPosition target, EntityPlayer player, World world) {
		...
		if (target.entityHit != null) {
			...
			if (id >= 0 && ItemAetherSpawnEgg.entityEggs.containsKey(id)) {
				result = new ItemStack(ItemsAether.aetherSpawnEgg, 1, id);
			}
		}
	...
	public void onBowPulled(FOVUpdateEvent event) {
		...
		if (item == ItemsAether.phoenixBow) {
	...
	public void onRenderInvisibility(RenderPlayerEvent.Pre event) {
		...
		if (playerAether != null) {
			if (playerAether.getAccessoryInventory().wearingAccessory(new ItemStack(ItemsAether.invisibilityCape))) {
