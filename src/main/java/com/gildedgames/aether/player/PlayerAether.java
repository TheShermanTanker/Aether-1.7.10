...
import com.gildedgames.aether.network.packets.*;
import com.gildedgames.aether.player.abilities.AbilityAccessories;
import com.gildedgames.aether.player.abilities.AbilityArmor;
import com.gildedgames.aether.player.abilities.AbilityFlight;
...
public class PlayerAether implements IPlayerAether {
	...
	private void activateParachute()
	{
		if (!this.player.capabilities.isCreativeMode)
		{
			...
			for (int i = 0; i < this.getEntity().inventory.getSizeInventory(); i++)
			{
				...
				if(stackInSlot != null && stackInSlot.getItem() == ItemsAether.cloudParachute)
				{
				...
				{
					if (stackInSlot != null && stackInSlot.getItem() == ItemsAether.goldenParachute)
					{
						itemstack = stackInSlot;
						break;
					}
			...
			if (itemstack != null)
			{
				if (itemstack.getItem() == ItemsAether.cloudParachute)
				{
				...
				{
					if (itemstack.getItem() == ItemsAether.goldenParachute)
					{
	...
	public void givePortalFrame()
	{
		...
		{
			this.player.inventory.addItemStackToInventory(new ItemStack(ItemsAether.aetherPortalFrame));
