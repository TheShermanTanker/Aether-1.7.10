...
public class AbilityAccessories implements IAetherAbility {
	...
	public void onUpdate() {
		if (this.player.getEntity().ticksExisted % 400 == 0) {
			this.player.getAccessoryInventory().damageWornStack(1, new ItemStack(ItemsAether.zaniteRing));
			this.player.getAccessoryInventory().damageWornStack(1, new ItemStack(ItemsAether.zanitePendant));
		}

		if (!this.player.getEntity().worldObj.isRemote && (this.player.getAccessoryInventory().wearingAccessory(new ItemStack(ItemsAether.iceRing)) || this.player.getAccessoryInventory().wearingAccessory(new ItemStack(ItemsAether.icePendant)))) {
				...
				for (int y = j - 1; y <= j + 1; y++) {
					for (int z = k - 1; z <= k + 1; z++) {
						...
						if (setBlock != null) {
							...
							this.player.getAccessoryInventory().damageWornStack(1, new ItemStack(ItemsAether.iceRing));
							this.player.getAccessoryInventory().damageWornStack(1, new ItemStack(ItemsAether.icePendant));
						}
					}
				}
			}
		...
		if (this.player.getAccessoryInventory().wearingAccessory(new ItemStack(ItemsAether.ironBubble))) {
			this.player.getEntity().setAir(0);
		}

		if (this.player.getAccessoryInventory().wearingAccessory(new ItemStack(ItemsAether.agilityCape))) {
		...
		if (this.player.getAccessoryInventory().wearingAccessory(new ItemStack(ItemsAether.invisibilityCape))) {
			this.player.getEntity().setInvisible(true);
			this.invisibilityUpdate = true;
		} else if (!this.player.getAccessoryInventory().wearingAccessory(new ItemStack(ItemsAether.invisibilityCape)) && !this.player.getEntity().isPotionActive(Potion.invisibility)) {
		...
		if (this.player.getAccessoryInventory().wearingAccessory(new ItemStack(ItemsAether.regenerationStone))) {
		...
		if (this.player.getAccessoryInventory().wearingAccessory(new ItemStack(ItemsAether.goldenFeather)) || this.player.getAccessoryInventory().wearingAccessory(new ItemStack(ItemsAether.valkyrieCape))) {
