...
import com.gildedgames.aether.entities.util.EntityHook;
import com.gildedgames.aether.inventory.InventoryAccessories;
...
import com.gildedgames.aether.network.packets.PacketAchievement;
import com.gildedgames.aether.player.abilities.AbilityRepulsion;
import com.gildedgames.aether.registry.achievements.AchievementsAether;
import com.gildedgames.aether.registry.achievements.AetherAchievement;
	...
	public void onAchievementGet(AchievementEvent event) {
		...
		if (!player.worldObj.isRemote && ((EntityPlayerMP) player).func_147099_x().canUnlockAchievement(achievement) && !((EntityPlayerMP) player).func_147099_x().hasAchievementUnlocked(achievement)) {
			if (event.achievement == AchievementsAether.enter_aether) {
				if (!player.inventory.addItemStackToInventory(new ItemStack(ItemsAether.loreBook))) {
					player.worldObj.spawnEntityInWorld(new EntityItem(player.worldObj, player.posX, player.posY, player.posZ, new ItemStack(ItemsAether.loreBook)));
				}

				if (!player.inventory.addItemStackToInventory(new ItemStack(ItemsAether.goldenParachute))) {
					player.worldObj.spawnEntityInWorld(new EntityItem(player.worldObj, player.posX, player.posY, player.posZ, new ItemStack(ItemsAether.goldenParachute)));
				}
			}
