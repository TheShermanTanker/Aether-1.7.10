...
import com.gildedgames.aether.network.packets.PacketSendEternalDay;
import com.gildedgames.aether.network.packets.PacketSendShouldCycle;
import com.gildedgames.aether.player.PlayerAether;
import com.gildedgames.aether.registry.achievements.AchievementsAether;
import com.gildedgames.aether.world.AetherData;
import com.gildedgames.aether.world.AetherWorldProvider;
...
AetherEventHandler {
	...
	public void onEntityInteract(EntityInteractEvent event) {
		if (event.target instanceof EntityAgeable) {
			...
			if (itemstack != null && itemstack.getItem() == ItemsAether.aetherSpawnEgg) {
		...
		if (event.target instanceof EntityCow || event.target instanceof EntityFlyingCow) {
			...
			if (heldItem != null && heldItem.getItem() == ItemsAether.skyrootBucket && EnumSkyrootBucketType.getType(heldItem.getItemDamage()) == EnumSkyrootBucketType.Empty) {
				if (!player.capabilities.isCreativeMode) {
					--heldItem.stackSize;
				}

				if (heldItem.stackSize <= 0) {
					player.inventory.setInventorySlotContents(player.inventory.currentItem, new ItemStack(ItemsAether.skyrootBucket, 1, EnumSkyrootBucketType.Milk.meta));
				} else if (!player.inventory.addItemStackToInventory(new ItemStack(ItemsAether.skyrootBucket, 1, EnumSkyrootBucketType.Milk.meta))) {
					player.dropPlayerItemWithRandomChoice(new ItemStack(ItemsAether.skyrootBucket, 1, EnumSkyrootBucketType.Milk.meta), false);
				}
			}
		}
	}
	...
	public void onFillBucket(FillBucketEvent event) {
		...
		boolean isWater = (!AetherConfig.activateOnlyWithSkyroot() && stack.getItem() == Items.water_bucket) || (stack.getItem() == ItemsAether.skyrootBucket && stack.getItemDamage() == 1);
			...
			if (source.getEntity() instanceof EntityPlayer) {
				...
				if (currentItem != null && currentItem.getItem() instanceof ItemSkyrootSword && !(entity instanceof EntityPlayer) && !(entity instanceof EntityWither) && !(entity instanceof EntityValkyrie)) {
					for (EntityItem items : event.drops) {
						...
						if (!(stack.getItem() instanceof ItemDungeonKey) && stack.getItem() != ItemsAether.victoryMedal && stack.getItem() != Items.skull) {
	...
	public void onEntityStruckByLightning(EntityStruckByLightningEvent event)
	{
		if (event.entity instanceof EntityPlayer)
		{
			...
			{
				if (player.inventory.getCurrentItem().getItem() == ItemsAether.lightningSword || player.inventory.getCurrentItem().getItem() == ItemsAether.lightningKnife)
				{
					event.setCanceled(true);
				}
			}
	...
	public void onEntityAttack(AttackEntityEvent event)
	{
		...
		{
			if (event.entityPlayer.getHeldItem().getItem() == ItemsAether.flamingSword)
			{
			...
			else if (event.entityPlayer.getHeldItem().getItem() == ItemsAether.pigSlayer)
			{
	...
	public boolean isGravititeTool(Item stackID) {
		return stackID == ItemsAether.gravititeShovel || stackID == ItemsAether.gravititeAxe || stackID == ItemsAether.gravititePickaxe;
	}
	...
	public void onWorldTick(TickEvent.WorldTickEvent event)
	{
		...
		for (Object entity : event.world.loadedEntityList)
		{
			if (entity instanceof EntityItem)
			{
				...
				if (entityItem.getEntityItem().getItem() == ItemsAether.dungeonKey)
				{
	...
	public void onFall(LivingFallEvent event)
	{
		if (event.entityLiving instanceof EntityPlayer)
		{
			...
			if (playerAether.getAccessoryInventory().wearingArmor(new ItemStack(ItemsAether.sentryBoots)) || playerAether.getAccessoryInventory().isWearingGravititeSet() || playerAether.getAccessoryInventory().isWearingValkyrieSet())
			{
				event.setCanceled(true);
			}
		}
